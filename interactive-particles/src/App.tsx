...
let density:number;
  ...
  constructor(x:number, y:number, size:number, color:string,){
    particleX = x + canvas.width/2-png.width*2,
    particleY = y + canvas.height/2-png.width*2,
    particleColor = color,
    baseX = x + canvas.width/2-png.width*2,
    baseY = y + canvas.height/2-png.width*2,
...
update() {
  ...
  // const [particleArray, setParticles] = useState([]);
    ...
    for (var y = 0, y2 = data.height; y < y2; y++) {
        for (var x = 0, x2 = data.width; x < x2; x++) {
            if (data.data[(y * 4 * data.width) + (x * 4) + 3] > 128) {
                ...
                particleArray.push(new Particle(positionX*4, positionY*4, size,color));
